...
const Notif = ({ icon: Icon, title, sub, accent = "blue", className = "", delay = 0 }) => (
  <motion.div
    ...
    className={`glass-strong rounded-2xl px-3.5 py-2.5 flex items-center gap-3 min-w-[210px] ${className}`}
  >
    <div
    ...
    >
      ...
      <div className="relative max-w-[1280px] mx-auto px-5 sm:px-8">
        ...
        <div className="mt-16 sm:mt-20 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          >
            <div className="relative rounded-3xl overflow-hidden h-[440px] sm:h-[520px] glass ring-glow">
              {/* Glow backlight */}
              <div className="absolute inset-0">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[420px] w-[420px] rounded-full radial-blue" />
                <div className="absolute bottom-0 right-0 h-[260px] w-[260px] rounded-full radial-purple" />
              </div>

              <img
                src={deviceImg}
                alt="Prosite on iPhone and MacBook"
              ...
              <div className="absolute top-5 right-5 flex flex-col gap-2.5">
                <Notif icon={Mail} title="New Client Inquiry" sub="Brand identity · ₹85,000 scope" delay={0.2} />
                <Notif icon={ShoppingBag} title="Product Sold" sub="Lightroom presets pack · ₹1,499" accent="purple" delay={0.4} />
                <Notif icon={Calendar} title="Event Ticket Booked" sub="Studio open day · seat #14" accent="gold" delay={0.6} />
              </div>
              <div className="absolute bottom-5 left-5 flex flex-col gap-2.5">
                <Notif icon={GraduationCap} title="Course Purchase" sub="Editorial Photography 101" accent="purple" delay={0.8} />
                <Notif icon={Users} title="Collaboration Request" sub="Studio Mira · Mumbai" delay={1} />
                <Notif icon={Bell} title="Your Prosite was viewed" sub="248 visits in last 24h" delay={1.2} />
              </div>

              {/* Label */}
              <div className="absolute top-5 left-5">
                <div className="glass-strong rounded-full px-3 py-1.5 text-[10.5px] uppercase tracking-[0.18em] text-emerald-300/90 flex items-center gap-2">
                  <Sparkles className="h-3 w-3" /> One Prosite · Everything
                </div>
              </div>
            </div>
          </motion.div>
